...
export const institucionesRelations = relations(instituciones, ({ one, many }) => ({
  tipoInstitucione: one(tipoInstituciones, {
    fields: [instituciones.tipoInstitucioneId],
    references: [tipoInstituciones.id]
  }),
  tipoBachillere: one(tipoBachilleres, {
    fields: [instituciones.tipoBachillereId],
    references: [tipoBachilleres.id]
  }),
  user: one(users, {
    fields: [instituciones.userId],
    references: [users.id]
  }),
}));
